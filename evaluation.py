...
class post(object):
	def __init__(self):
		...
		self.precision_list = [None for idx in xrange(len(self.label2idx))]
		self.recall_list = [None for idx in xrange(len(self.label2idx))]
		# TODO : Remove this
		self.precision_list_mm = [None for idx in xrange(len(self.label2idx))]
		self.recall_list_mm = [None for idx in xrange(len(self.label2idx))]
	...
	def longest_common_subsequence(self,post1, post2):
		...
		n = len(post1)
		m = len(post2)
		lcs = [[0 for idx in xrange(m+1)] for idx in xrange(n+1)]
	...
	def removeDuplicates(self, instance_list):	
		new_instance_list = []
		# pdb.set_trace()		
		add_to_list = [True for idx in xrange(len(instance_list))]
		for idx in xrange(len(instance_list) - 1):		
			for jdx in xrange(idx+1, len(instance_list)):
				lcs = self.longest_common_subsequence(instance_list[idx], instance_list[jdx])			
				if lcs == len(instance_list[idx].split(' ')):
					add_to_list[idx] = False
				elif lcs == len(instance_list[jdx].split(' ')):
					add_to_list[jdx] = False
		for idx in xrange(len(instance_list)):
			if add_to_list[idx]:
	...
	def set_values(self,post,prediction):
		...
		self.gold_instances = [[] for idx in xrange(len(self.label2idx))]
		self.prediction_instances = [[] for idx in xrange(len(self.label2idx))]
		curr_gold_label,curr_gold_instance, curr_predicted_label, curr_predicted_instance = "","","",""
		
		for idx in xrange(len(post)):
		...
		for idx in xrange(len(self.prediction_instances)):			
			if self.mode == 'multi_match':
				# Multi match
				self.precision_list[idx] = self.multi_match(self.prediction_instances[idx], self.gold_instances[idx], True)
				self.recall_list[idx] = self.multi_match(self.prediction_instances[idx], self.gold_instances[idx], False)
				self.precision_list_mm[idx] = self.maximal_match(self.prediction_instances[idx], self.gold_instances[idx], True)
				self.recall_list_mm[idx] = self.maximal_match(self.prediction_instances[idx], self.gold_instances[idx], False)
			elif self.mode == 'max_match':
				# Maximal match 
				self.precision_list[idx] = self.maximal_match(self.prediction_instances[idx], self.gold_instances[idx], True)
				self.recall_list[idx] = self.maximal_match(self.prediction_instances[idx], self.gold_instances[idx], False)
			else:
				raise ValueError
	...
	def multi_match(self, pred_list, gold_list, precision = True):
		...
		if m == 0:
			if precision:
				return [(0.,pred_list[idx],'') for idx in xrange(n)]
			else:
				return []
		# 1.2 no pred:
		if n == 0:
			if precision :
				...
				return [(0.,'',gold_list[idx]) for idx in xrange(m)]
		if precision:
			ret = []
			for idx in xrange(n):
				max_match = -1
	...
	def maximal_match(self, pred_list, gold_list, precision = True):
		...
		edges = [[0. for idx in xrange(m)] for jdx in xrange(n)]
		for idx in xrange(n):
			for jdx in xrange(m):
	...
	def maximal_match_helper(self, edges, pred_list, gold_list, row_bool):
		...
		m = munkres.Munkres()
		cost = [[large_val for idx in xrange(n)] for idx in xrange(n)]
		for idx in xrange(act_row):
			for jdx in xrange(act_col):
...
class evaluator(object):
	def __init__(self, pred_file_name, gold_file_name, verbosity = True):				
		...
		self.posts = [None for idx in xrange(len(posts))]
		idx = 0
	...
	def compute_averages(self):		
		...
		self._precision_list = [0. for idx in xrange(len(label2idx))]
		self._recall_list = [0. for idx in xrange(len(label2idx))]
		self.average_per_post_precision_list = [0. for idx in xrange(len(label2idx))]
		self.average_per_post_recall_list = [0. for idx in xrange(len(label2idx))]
		for idx in xrange(len(label2idx)):
			macro_count_prec = 0
			macro_count_recall = 0
			micro_count_prec = 0
			micro_count_recall = 0
			for post in self.posts:
				x = sum( map( lambda x: x[0] , post.precision_list[idx]) )
				xl = len(post.precision_list[idx])
				y = sum( map( lambda x: x[0] , post.recall_list[idx])  )
	...
	def __str__(self):
		...
		if self.temporal:
			...
			if (self._precision_list[3] + self._recall_list[3] ) != 0.:
				write_buf += '\t FSCORE: %.4f\n'%(2 * self._precision_list[3] * self._recall_list[3] / (self._precision_list[3] + self._recall_list[3] ) )
			else:
		...
		write_buf += '\t FSCORE: %.4f\n'%(2 * self.average_per_post_precision_list[0] * self.average_per_post_recall_list[0] / (self.average_per_post_precision_list[0] + self.average_per_post_recall_list[0] ) )

		write_buf += 'ATTR: \n'
		write_buf += '\t   PREC: %.4f\n'%self.average_per_post_precision_list[1]
		write_buf += '\t    REC: %.4f\n'%self.average_per_post_recall_list[1]
		write_buf += '\t FSCORE: %.4f\n'%(2 * self.average_per_post_precision_list[1] * self.average_per_post_recall_list[1] / (self.average_per_post_precision_list[1] + self.average_per_post_recall_list[1] ) )

		write_buf += 'LOCATION: \n'
		write_buf += '\t   PREC: %.4f\n'%self.average_per_post_precision_list[2]
		write_buf += '\t    REC: %.4f\n'%self.average_per_post_recall_list[2]
		write_buf += '\t FSCORE: %.4f\n'%(2 * self.average_per_post_precision_list[2] * self.average_per_post_recall_list[2] / (self.average_per_post_precision_list[2] + self.average_per_post_recall_list[2] ) )
		if self.temporal:
			write_buf += 'TEMPORAL: \n'
			...
			if self.average_per_post_precision_list[3] + self.average_per_post_recall_list[3]  != 0.:
				write_buf += '\t FSCORE: %.4f\n'%(2 * self.average_per_post_precision_list[3] * self.average_per_post_recall_list[3] / (self.average_per_post_precision_list[3] + self.average_per_post_recall_list[3] ) )
			else:
				write_buf += '\t FSCORE: NAN\n'
		write_buf += '-------------------------------------------------------\n' 
		return write_buf
...
if __name__=="__main__":
	...
	parser.add_argument('-p_file', dest='p_file', metavar = 'Prediction File', type = str, default='', help = 'The Prediction File')
	parser.add_argument('-g_file', dest='g_file', metavar = 'Gold File', type = str, default='', help = 'The Gold File')	
	parser.add_argument('-s_file', dest = 'save_file', metavar = 'Save File', type = str, default = '', help = 'File to save in')
	parser.add_argument('-verbose', dest = 'verbose', metavar = 'Verbosity', type = str, default = 'True', help = 'Info to print')
